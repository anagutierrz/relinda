...
function renderCart(){const count=$('#cartCount'),items=$('#cartItems'),total=$('#cartTotal'),checkout=$('#checkoutBtn'); if(!count||!items)return; const normalized=cart.map(i=>({name:i.name, price:Number(i.price)||0, qty:Number(i.qty)||1})); cart=normalized; const countNum=normalized.reduce((a,i)=>a+(Number(i.qty)||1),0); count.textContent=String(countNum); items.innerHTML=normalized.length?normalized.map(i=>`<div class="cart-item"><div><b>${i.name}</b><br><small>${money(i.price)} x ${i.qty}</small></div><button data-remove="${i.name}">×</button></div>`).join(''):'<p>Tu carrito está vacío.</p>'; const t=normalized.reduce((a,i)=>a+((Number(i.price)||0)*(Number(i.qty)||1)),0); total.textContent=money(t); const text=encodeURIComponent('Hola Relinda, quiero comprar: '+normalized.map(i=>`${i.qty} x ${i.name}`).join(', ')+` Total: ${money(t)}`); checkout.href='https://wa.me/?text='+text; $$('[data-remove]').forEach(b=>b.onclick=()=>{cart=cart.filter(i=>i.name!==b.dataset.remove);saveCart()})}
...
let heroIndex = 0;
function setHero(index){ if(!heroProducts.length) return; heroIndex = index % heroProducts.length; heroProducts.forEach((img,i)=>img.classList.toggle('active', i===heroIndex)); heroButtons.forEach((btn,i)=>btn.classList.toggle('active', i===heroIndex)); }
heroButtons.forEach(btn=>btn.addEventListener('click',()=>setHero(Number(btn.dataset.hero))));
if(heroProducts.length){ setInterval(()=>setHero((heroIndex+1)%heroProducts.length), 3200); }
...
const WHEEL_DATA = {
  longevity: { benefit: 'Longevidad', title: 'Wellness by Relinda', desc: 'Una visión de bienestar femenino que busca acompañar tu estilo de vida a largo plazo.', img: 'relinda-logo.png', link: 'about.html', accent: '#CF9AA4' },
  melatonin: { benefit: 'Sueño profundo', title: 'Melatonin Gummies', desc: 'Apoya el descanso, ayuda a conciliar el sueño y a mantenerlo durante la noche.', img: 'product-melatonin.png', link: 'product-melatonin.html', accent: '#A24AE6' },
  adult: { benefit: 'Energía diaria', title: 'Adult Multivitamins', desc: 'Soporte diario para energía, inmunidad y bienestar en mujeres activas.', img: 'product-adult.png', link: 'product-adult-multivitamins.html', accent: '#F38B47' },
  biotin: { benefit: 'Cabello, piel y uñas', title: 'Biotin Collagen Keratin', desc: 'Fórmula enfocada en fortalecer cabello, uñas y mejorar la apariencia de la piel.', img: 'product-biotin.png', link: 'product-biotin-collagen-keratin.html', accent: '#37C8DE' }
};
...
function setWheelProduct(key){
  const data = WHEEL_DATA[key]; if(!data || !wheelCenterProduct) return;
  wheelSegments.forEach(seg => seg.classList.toggle('active', seg.dataset.target === key));
  wheelCenterProduct.style.opacity = '0'; wheelCenterProduct.style.transform = 'translateY(12px) scale(.96)';
  setTimeout(()=>{ wheelCenterProduct.src = data.img; wheelCenterProduct.alt = data.title; wheelTitle.textContent = data.title; wheelDesc.textContent = data.desc; wheelBenefit.textContent = data.benefit; wheelLink.href = data.link; wheelScene?.style.setProperty('--wheel-accent', data.accent); wheelCenterProduct.style.opacity = '1'; wheelCenterProduct.style.transform = ''; }, 160);
}
wheelSegments.forEach(seg => { seg.addEventListener('mouseenter', ()=>setWheelProduct(seg.dataset.target)); seg.addEventListener('click', ()=>setWheelProduct(seg.dataset.target)); });
if(wheelScene){
  wheelScene.addEventListener('mousemove', (e)=>{ if(window.innerWidth < 900) return; const r = wheelScene.getBoundingClientRect(); const x = ((e.clientX - r.left) / r.width) - 0.5; const y = ((e.clientY - r.top) / r.height) - 0.5; wheelScene.style.setProperty('--rx', `${y * -7}deg`); wheelScene.style.setProperty('--ry', `${x * 10}deg`); wheelScene.style.setProperty('--tx', `${x * 8}px`); wheelScene.style.setProperty('--ty', `${y * 8}px`); document.querySelectorAll('.wheel-float').forEach((el, i) => { const mx = (i % 2 === 0 ? 1 : -1) * x * (14 + i * 2); const my = (i % 2 === 0 ? -1 : 1) * y * (12 + i * 2); el.style.transform = `translate(${mx}px, ${my}px)`; }); });
  wheelScene.addEventListener('mouseleave', ()=>{ wheelScene.style.setProperty('--rx', '0deg'); wheelScene.style.setProperty('--ry', '0deg'); wheelScene.style.setProperty('--tx', '0px'); wheelScene.style.setProperty('--ty', '0px'); document.querySelectorAll('.wheel-float').forEach(el => el.style.transform = ''); });
  const syncWheelScroll = () => { const rect = wheelScene.getBoundingClientRect(); const view = window.innerHeight || document.documentElement.clientHeight; const progress = Math.max(0, Math.min(1, (view - rect.top) / (view + rect.height))); wheelScene.style.setProperty('--spin', `${(progress - 0.5) * 10}deg`); };
  syncWheelScroll();
